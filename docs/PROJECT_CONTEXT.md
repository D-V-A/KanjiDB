...
Current version: **0.3.2-alpha**, `versionCode = 8`. Both values are set manually in [app/build.gradle.kts](../app/build.gradle.kts); there is no automatic derivation from Git or build date. About reads the installed package's versionName through PackageManager. versionName is the display version; versionCode is the Android update sequence and should increase for subsequent distributed updates. Neither currently versions the dictionary.
...
The destination shares a saveable Material 3 enter-always scroll behavior for its title, tab strip and active-page controls. `CollapsingCollectionHeader` measures this block before the pager and gives the grid the remaining viewport as the header slides out/in. User nested-scroll deltas reveal it immediately in either collection, without index-based visibility rules or compensating grid scroll calls. Programmatic selection clearance is excluded from header scrolling; vertical gesture release uses the Material fling/snap behavior. Every tab change (tap or swipe), including returning to a previous tab, creates fully expanded header state. Only the header state is keyed to the current page; pager/grid state and section expansion are unaffected. Details -> Back on the same tab still restores the header state. My Lists displays the header because its placeholder cannot scroll vertically. The floating selection panel stays at the viewport bottom, independently of the header, and the app bottom navigation remains outside this layout. Group options/rules dialog state live at destination level so moving the controls does not recreate their state. The initial-data grid measurement guards remain unchanged.
